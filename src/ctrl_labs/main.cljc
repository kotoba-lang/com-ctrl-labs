(ns ctrl_labs.main
  "Kotodama WASM entrypoint for the Ctrl_labs clean-room actor (L4) — Clojure port.
  ...
  facts (`ctrl_labs.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "ctrl_labs")
(def tier "L4")
...
(def entity-specs
  [{:entity "Device"      :plural "devices"      :id-prefix "ctrllabs_dev"
    :fields [:model :channels :samplingHz :firmware] :required [:model :channels]
    :coerce {:channels :int :samplingHz :int}   :refs {}}
   {:entity "Session"     :plural "sessions"     :id-prefix "ctrllabs_ses"
...
(defn emit-facts
  "EAVT facts for one record: {\"ctrl_labs.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ctrl_labs-compat" :tier tier :entities entities} 200])
...
(defn start! [] :ctrl_labs-compat/ready)
